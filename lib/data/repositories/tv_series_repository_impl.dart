...
class TvSeriesRepositoryImpl implements TvSeriesRepository {
  ...
  Future<Either<Failure, List<TvSeries>>> getPopularTvSeries() async {
    try {
      final result = await remoteDataSource.getTvSeriesPopuler();
      return Right(result.map((model) => model.toEntity()).toList());
    } on ServerException {
      return Left(ServerFailure(''));
    } on SocketException {
  ...
  Future<Either<Failure, List<TvSeries>>> getTvSeriesRecommendations(
      int id) async {
    try {
      final result = await remoteDataSource.getTvSeriesRecommendationsById(id);
      return Right(result.map((model) => model.toEntity()).toList());
    } on ServerException {
      return Left(ServerFailure(''));
    } on SocketException {
  ...
  Future<Either<Failure, String>> saveWatchlist(TvSeriesDetail tvSeries) async {
    try {
      ...
      final result = await localDataSource.insertWatchlist(tvSeriesTable);
      return Right(result);
  ...
  Future<Either<Failure, String>> removeWatchlist(
    ...
    try {
      final result = await localDataSource.deleteTvSeriesWatchList(tvSeries.id);
      return Right(result);
    } on DatabaseException catch (e) {
      return Left(DatabaseFailure(e.message));
    } catch (e) {
  ...
  Future<bool> isAddedToWatchlist(int id) async {
    try {
      final result = await localDataSource.isInWatchlist(id);
      return result;
    } catch (e) {
      return false;
    }
